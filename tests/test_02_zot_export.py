...
def test_make_asreview_csv(tmp_path: Path, filename):
    ...
    # Act
    make_asreview_csv(zotero_csv=input_file, out_csv=output_file, library_id=os.getenv("ZOTERO_LIBRARY_ID"), deduplicate=False)
...
def test_make_asreview_csv_from_db(tmp_path: Path):
    ...
    # Act
    make_asreview_csv_from_db(db_path=db_file, out_csv=output_file, library_id=os.getenv("ZOTERO_LIBRARY_ID"), deduplicate=False)
    print(f"Output file saved at: {output_file}")
